...
/***************************************************************************
  ...
  说    明：
 ***************************************************************************/
bool allzero(char pure_decimal[],int count)
{
...
void input_pure_decimal(char pure_decimal[])
{
	...
	while (true) {
		...
		while (count < N_NUM ) {
			...
			if (a == '\n')
				break;
			pure_decimal[count++] = a;
		}

		int count2 = 0;
		for (int i = 0; i <= count; i++)
		{
			if (i==0 && pure_decimal[i] !='.') {
				cout << "不是以.开头，请重输" << endl;
				break;
			}
			if (i!=0 && pure_decimal[i] != '1' && pure_decimal[i] != '0') {
			...
			bool m = allzero(pure_decimal, count);
			if (m) {
				cout << "输入为全0，请重输" << endl;
				break;
			}
